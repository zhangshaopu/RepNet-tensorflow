...
class ResnetPeriodEstimator(tf.keras.models.Model):
  ...
  def call(self, x):
    # Ensures we are always using the right batch_size during train/eval. x.shape [20, 64, 112, 112, 3]
    batch_size = tf.shape(x)[0] # 20
    # Conv Feature Extractor.
    x = tf.reshape(x, [-1, self.image_size, self.image_size, 3]) # [1280, 112, 112, 3]
    x = self.base_model(x) # [1280, 7, 7, 1024] 经过resnet后
    h = tf.shape(x)[1] # 7
    w = tf.shape(x)[2] # 7
    c = tf.shape(x)[3] # 1024
    x = tf.reshape(x, [batch_size, -1, h, w, c]) # [20, 64, 7, 7, 1024]

    # 3D Conv to give temporal context to per-frame embeddings. 
    for bn_layer, conv_layer in zip(self.temporal_bn_layers,
                                    self.temporal_conv_layers):
      x = conv_layer(x) # [20, 64, 7, 7, 512]
      x = bn_layer(x) # [20, 64, 7, 7, 512]
      x = tf.nn.relu(x) # [20, 64, 7, 7, 512]

    x = tf.reduce_max(x, [2, 3]) # [20, 64, 512]

    # Reshape and prepare embs for output.
    final_embs = x # [1, 64, 512] 

    # Get self-similarity matrix.
    x = get_sims(x, self.temperature) # [1, 64, 64, 1] temperature=13.544,

    # 3x3 conv layer on self-similarity matrix.
    x = self.conv_3x3_layer(x) # [1, 64, 64, 32]
    x = tf.reshape(x, [batch_size, self.num_frames, -1]) # [1,64,2048]
    within_period_x = x

    # Period prediction.
    x = self.input_projection(x) # [1, 64, 512]
    x += self.pos_encoding # [1, 64, 512]
    for transformer_layer in self.transformer_layers:
      x = transformer_layer(x) # [1, 64, 512] 
    x = flatten_sequential_feats(x, batch_size, self.num_frames) # [1, 64, 512]
    for fc_layer in self.fc_layers:
      x = self.dropout_layer(x) # [1, 64, 512]
      x = fc_layer(x) # [1, 64, 512]
    # [1,64,32]
    # Within period prediction.
    within_period_x = self.input_projection2(within_period_x) # [1, 64, 512]
    within_period_x += self.pos_encoding2 # [1, 64, 512]
    ...
    return x, within_period_x, final_embs # [1, 64, 32]  [1, 64, 1] [1, 64, 512]
...
def viz_reps(frames,
  ...
  tmp_path = 'E://Code//repnet_ckpt//output.mp4'
...
def get_counts(model, frames, strides, batch_size,
  ...
  frames = model.preprocess(frames) # [239, 112, 112, 3]

  for stride in strides:
    num_batches = int(np.ceil(seq_len/model.num_frames/stride/batch_size)) # 什么意思 为什么要除batch_size
    ...
    for batch_idx in range(num_batches):
      idxes = tf.range(batch_idx*batch_size*model.num_frames*stride,
                       ...
                       stride)
      idxes = tf.clip_by_value(idxes, 0, seq_len-1) # (1280,) 1280 / 30 = 42 总共可以计算40多秒30帧的视频 这里总共300帧，所以299之后的都是最后一帧的数据
      curr_frames = tf.gather(frames, idxes)
      curr_frames = tf.reshape(
          curr_frames,
          [batch_size, model.num_frames, model.image_size, model.image_size, 3]) # [20, 64, 112, 112, 3]
